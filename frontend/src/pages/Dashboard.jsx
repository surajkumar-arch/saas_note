...
export default function Dashboard({ token, user, onLogout }){
  ...
  useEffect(()=> { fetchNotes(); fetchTenant(); }, []);

  async function fetchTenant(){
    try {
      // derive tenant info by fetching notes and checking plan via notes endpoint owner
      // better: no tenant endpoint provided; infer from notes or stored user
      const res = await axios.get(apiBase + '/api/notes', { headers });
      setNotes(res.data);
      // no tenant plan endpoint, so we will derive by counting notes and user role
    } catch(e){
      console.error(e);
    }
  }
  ...
  async function upgrade(){
    ...
    }catch(e){ setInfo(e.response?.data?.error || 'Upgrade failed'); }
    fetchNotes();
  }
  ...
  return (
    <div className="min-h-screen">
      <div className="bg-white shadow p-4 flex justify-between items-center">
        <div className="font-bold">SaaS Notes - {user.tenant}</div>
        ...
        <div className="card mb-4">
          ...
          <form onSubmit={createNote} className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              {user.role === 'admin' && freeLimitReached && <button type="button" className="btn btn-secondary" onClick={upgrade}>Upgrade to Pro</button>}
            </div>
          </form>
        </div>
          ...
          {notes.map(n=>(
            <div key={n.id} className="note-card">
              ...
              <div className="mt-3 flex gap-2">
                <button className="btn btn-secondary" onClick={()=>deleteNote(n.id)}>Delete</button>
              </div>
            </div>
          ))}
        </div>

        {notes.length >= 3 && user.role !== 'admin' && (
          <div className="mt-4 p-3 bg-yellow-50 border rounded">Free plan limit reached. Ask your admin to upgrade to Pro.</div>
        )}
      </div>
    </div>
  );
